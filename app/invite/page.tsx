      ...
      <div className="bg-white border border-[#E5E7EB] rounded-lg w-full max-w-md p-8 text-center">
        <div className="flex items-center gap-3 mb-8 justify-center">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-[#6B7280]">Team Invitation</p>
          </div>
        </div>
        ...
        {state === "not_found" && (
          <>
            ...
            <h2 className="text-xl font-bold text-[#1F2937] mb-2">Invitation not found</h2>
            <p className="text-[#6B7280] text-sm mb-6">This invitation link is invalid or has already been used.</p>
        ...
        {state === "expired" && (
          <>
            <XCircle size={40} className="text-red-500 mx-auto mb-3" />
            <h2 className="text-xl font-bold text-[#1F2937] mb-2">Invitation expired</h2>
            <p className="text-[#6B7280] text-sm mb-6">This invitation link has expired. Ask the business owner to send a new invite.</p>
            <Link href="/login" className="btn btn-primary w-full">Go to Login</Link>
          </>
        )}
        ...
        {state === "found" && invite && (
          <>
            ...
            <h2 className="text-xl font-bold text-[#1F2937] mb-2">You&apos;re invited!</h2>
            <p className="text-[#6B7280] text-sm mb-1">
              <span className="font-semibold text-[#1F2937]">{invite.invited_by}</span> invited you to join
            </p>
            <p className="text-brand-navy font-bold text-lg mb-1">{invite.business_name}</p>
            <p className="text-[#6B7280] text-sm mb-6">
              as <span className="font-semibold text-[#374151]">{roleLabel(invite.role)}</span>
            </p>
            ...
            {me ? (
              ...
              <div className="space-y-3">
                <p className="text-[#6B7280] text-sm">Log in or register to accept this invitation.</p>
        ...
        {state === "accepted" && (
          <>
            ...
            <h2 className="text-xl font-bold text-[#1F2937] mb-2">Welcome aboard!</h2>
            <p className="text-[#6B7280] text-sm">You&apos;ve joined the business. Redirecting to dashboard…</p>
          </>
        )}
        ...
        {state === "wrong_email" && (
          <>
            <XCircle size={40} className="text-red-500 mx-auto mb-3" />
            <h2 className="text-xl font-bold text-[#1F2937] mb-2">Wrong account</h2>
            <p className="text-[#6B7280] text-sm mb-6">
        ...
        {state === "error" && (
          <>
            ...
            <p className="text-[#6B7280] text-sm mb-6">Please try again or contact support.</p>
            <Link href="/login" className="btn btn-primary w-full">Go to Login</Link>
          </>
        )}
      </div>
